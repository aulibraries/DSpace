...
public class AuthorizeServiceImpl implements AuthorizeService {
    ...
    public void authorizeAction(Context c, EPerson e, DSpaceObject o, int action, boolean useInheritance)
        ...
        if (!authorize(c, o, action, e, useInheritance)) {
            ...
            throw new AuthorizeException("Authorization denied for action " + actionText + " on "
                    + Constants.typeText[otype] + ":" + oid + " by user " + userid, o, action);
        }
    }
     ...
     * object. Always returns true if the ignore authorization flat is set in the
     * current context.
     *
     * @param c              current context. User is irrelevant; "ignore
     *                       authorization" flag is relevant
     ...
     * @param useInheritance flag to say if ADMIN action on the current object or
     *                       parent object can be used
     * @return <code>true</code> if user is authorized to perform the given action,
    ...
    protected boolean authorize(Context c, DSpaceObject o, int action, EPerson e, boolean useInheritance)
        ...
        if (e != null) {
            ...
            DSpaceObject adminObject = useInheritance
        ...
        for (ResourcePolicy rp : getPoliciesActionFilter(c, o, action)) {
            if (ignoreCustomPolicies && ResourcePolicy.TYPE_CUSTOM.equals(rp.getRpType())) {
                if (c.isReadOnly()) {
                    // When we are in read-only mode, we will cache authorized actions in a
                    // different way
    ...
    public boolean isCommunityAdmin(Context c) throws SQLException {
        ...
        if (e != null) {
            List<ResourcePolicy> policies = resourcePolicyService.find(c, e, groupService.allMemberGroups(c, e),
    ...
    @Override
    public void addPolicy(Context context, DSpaceObject o, int actionID, EPerson e, String type)
    ...
    public void inheritPolicies(Context c, DSpaceObject src, DSpaceObject dest)
        ...
        // Only inherit non-ADMIN policies (since ADMIN policies are automatically
        // inherited)
    ...
    /**
     * Generate Policies policies READ for the date in input adding reason. New
     * policies are assigned automatically at the groups that have right on the
     * collection. E.g., if the anonymous can access the collection policies are
        ...
        if (embargoDate != null || (embargoDate == null && dso instanceof Bitstream)) {
            ...
            if (!isAnonymousInPlace) {
                ...
                ResourcePolicy rp = createOrModifyPolicy(null, context, null,
                        groupService.findByName(context, Group.ANONYMOUS), null, embargoDate, Constants.READ, reason,
                        dso);
                if (rp != null)
                    resourcePolicyService.update(context, rp);
            }
    ...
    public ResourcePolicy createOrModifyPolicy(ResourcePolicy policy, Context context, String name, Group group,
            EPerson ePerson, Date embargoDate, int action, String reason, DSpaceObject dso)
            throws AuthorizeException, SQLException {
        ResourcePolicy policyTemp = null;
        if (policy != null) {
            List<ResourcePolicy> duplicates = resourcePolicyService.findByTypeGroupActionExceptId(context, dso, group,
                    action, policy.getID());
            if (!duplicates.isEmpty()) {
                policy = duplicates.get(0);
            }
        } else {
            // if an identical policy (same Action and same Group) is already in place
            // modify it...

...
public class DescribeStep extends AbstractProcessingStep
{
        ...
        AuthorizeException
    {
        ...
        {
            wsiID = wsi.getID();
            currentStage = wsi.getStageReached();
        }

        log.debug(LogManager.getHeader(context, "Workspace Item", "Item ID: "+item.getID()+" Workspace Item ID:  "+String.valueOf(wsiID)));
        log.debug(LogManager.getHeader(context, "Current stage", String.valueOf(currentStage)));
        ...
        for (DCInput input : inputs)
        {
            ...
            if (!input.isVisible(subInfo.isInWorkflow() ? DCInput.WORKFLOW_SCOPE : DCInput.SUBMISSION_SCOPE))
            {
                continue;
            }
            if (input.getInputType().equals("qualdrop_value"))
            {
                ...
                for (int j = 0; j < pairs.size(); j += 2)
                {
                    ...
                    item.clearMetadata(input.getSchema(), input.getElement(), qualifier, Item.ANY);
                }
            }
            else
            {
        ...
        for (int j = 0; j < inputs.length; j++)
        {
            ...
            if (!inputs[j].isVisible(subInfo.isInWorkflow() ? DCInput.WORKFLOW_SCOPE : DCInput.SUBMISSION_SCOPE))
            {
                continue;
            }
            ...
            else if (inputType.equals("dropdown") || inputType.equals("list") || (cmgr.isChoicesConfigured(fieldKey) &&
            ...
            {
                List<String> quals = getRepeatedParameter(request, schema + "_" + element, schema + "_" + element + "_qualifier");
                List<String> vals = getRepeatedParameter(request, schema + "_" + element, schema + "_" + element + "_value");
                for (int z = 0; z < vals.size(); z++)
                {
                    ...
                    if (!buttonPressed.equals("submit_" + schema + "_" + element + "_remove_" + z) && !thisVal.equals(""))
                    {
        ...
        {
            String pageStr = buttonPressed.substring(buttonPressed.lastIndexOf("_")+1, buttonPressed.length()).substring(0,
                            1);
            log.debug(LogManager.getHeader(context, "Current stage  ", String.valueOf(currentStage)));
            log.debug(LogManager.getHeader(context, "Jumping to page ", pageStr));
            if(null != pageStr)
            {
                jumpToPage = Integer.parseInt(pageStr);
            }
        }
       ...
       /**
         * Only perform the following actions if the user clicks
         * the Next button or a progress bar button that advances
         * them to the next page in the submission process.
         *
         * Note: Removed the condition buttonPressed.equals(PREVIOUS_BUTTON)
         * in order to correct a UX flaw. Users should always be allowed to
         * navigate backwards in the submission workflow process.
         *
         */
        if (buttonPressed.equals(NEXT_BUTTON) || (buttonPressed.startsWith(PROGRESS_BAR_PREFIX) && (jumpToPage > currentStage)))
        {

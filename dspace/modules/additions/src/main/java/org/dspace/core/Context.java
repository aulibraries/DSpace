...
public class Context
{
    ...
    public void turnOffAuthorisationSystem()
    {
        ...
        if (log.isDebugEnabled())
        {
            ...
            log.debug(LogManager.getHeader(this, "turn_off_auth_system ", " Calling class name = "+caller));

            authStateClassCallHistory.push(caller);

            log.debug(LogManager.getHeader(this, "turn_off_auth_system ", " Size of authStateCallCallHistory = "+String.valueOf(authStateClassCallHistory.size())));
        }
        ignoreAuth = true;
    }
    ...
    public void restoreAuthSystemState()
    {
        ...
        if (log.isDebugEnabled())
        {
            ...
            log.debug(LogManager.getHeader(this, "restore_auth_sys_state ", " Is authStateClassCallHistory empty? "+String.valueOf((authStateClassCallHistory.isEmpty()))));

            String previousCaller = null;

            if (authStateClassCallHistory.size() > 0) {
                previousCaller = (String) authStateClassCallHistory.pop();
            }

            log.debug(LogManager.getHeader(this, "restore_auth_sys_state ", " Is previousCaller null? "+String.valueOf(StringUtils.isBlank(previousCaller))));

            // if previousCaller is not the current caller *only* log a warning
            if (StringUtils.isNotBlank(previousCaller) && !previousCaller.equals(caller)) {
                log.warn(LogManager.getHeader(this,

...
<xsl:stylesheet
	...
	<!-- Custom template -->
	<!-- Replace any dri:field's @rend attribute value if the node's parent node name is dri:list and the node's 
			@id attribute value matches a specified phrase, and the dri:field's @type attribute value equals 'button' -->
    <xsl:template match="dri:list[@id='aspect.submission.StepTransformer.list.submit-progress']/dri:item/dri:field[@type='button']">
        <field>
            ...
            <xsl:attribute name="rend">
                ...
                <xsl:text>progress-btn btn-success</xsl:text>
            </xsl:attribute>
            <xsl:apply-templates/>
        </field>
    </xsl:template>

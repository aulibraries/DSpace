...
public class UploadStep extends AbstractSubmissionStep {
    ...
    protected static final Message T_virus_checker_error = message(
    ...
    protected static final Message AUETD_INVALID_FILE_FORMAT_ERROR = message(
            "xmlui.Submission.submit.UploadStep.AUETD_file_upload_invalid_file_format_error");
    protected static final Message AUETD_FILE_UPLOAD_FORM_HEAD = message(
            "xmlui.Submission.submit.UploadStep.AUETD_file_upload_form_head");
    protected static final Message AUETD_FILE_UPLOAD_HELP = message(
            "xmlui.Submission.submit.UploadStep.AUETD_file_upload_field_help");
    protected static final Message AUETD_FILE_UPLOAD_SUBMIT_BUTTON_NAME = message(
            "xmlui.Submission.submit.UploadStep.AUETD_file_upload_submit_button_name");
    protected static final Message AUETD_SUBMIT_REMOVE_BUTTON_NAME =
        message("xmlui.Submission.submit.UploadStep.AUETD_submit_remove_button_name");

    /**
     * Global reference to edit file page (this is used when a user requests to edit
     * a bitstream)
     **/
    ...
    public void addBody(Body body)
        ...
        boolean disableFileEditing = (submissionInfo.isInWorkflow()) && !DSpaceServicesFactory.getInstance()
                .getConfigurationService().getBooleanProperty("workflow.reviewer.file-edit");
        java.util.List<Bundle> bundles = itemService.getBundles(item, Constants.CONTENT_BUNDLE_NAME);
        ...
        // Part A:
        // First ask the user if they would like to upload a new file (may be the first
        // one)
        ...
        List upload = null;
        if (bitstreams.isEmpty() && !disableFileEditing) {
            ...
            file.setLabel(AUETD_FILE_UPLOAD_FORM_HEAD);
            file.setHelp(AUETD_FILE_UPLOAD_HELP);
            file.setRequired();

            if (submissionInfo.containsKey(AUETDConstants.AUETD_FILE_UPLOAD_ERROR_KEY)
                    && StringUtils.isNotBlank(submissionInfo.get(AUETDConstants.AUETD_FILE_UPLOAD_ERROR_KEY).toString())) {
                this.errorFlag = (int) submissionInfo.get(AUETDConstants.AUETD_FILE_UPLOAD_ERROR_KEY);
            }
            ...
            renderUploadedFileSummary(bitstreams, div, item);
    ...
    public void make_sherpaRomeo_submission(Item item, Division divIn) throws WingException {
        ...
        if (DSpaceServicesFactory.getInstance().getConfigurationService()
                .getBooleanProperty("webui.submission.sherparomeo-policy-enabled", true)) {
            ...
            if (sherpaSubmitService.hasISSNs(context, item)) {
                ...
                // Since sherpa web service doesn't work reliable with more than 1 issn, perform
                // the service for each issn
                ...
                while (issnsIterator.hasNext()) {
                    ...
                    if (CollectionUtils.isNotEmpty(journals)) {
                        for (SHERPAJournal journ : journals) {
                            ...
                            sherpaList.addItem().addFigure(
                                    contextPath + "/static/images/" + (i == 0 ? "romeosmall" : "clear") + ".gif",
                                    "http://www.sherpa.ac.uk/romeo/", "sherpaLogo");
                            ...
                            if (CollectionUtils.isNotEmpty(publishers)) {
                                ...
                                sherpaList.addItem().addHighlight("sherpaStyle " + pub.getRomeocolour())
                                        .addContent(message("xmlui.aspect.sherpa.submission." + pub.getRomeocolour()));
                            }

                            sherpaList.addItem().addXref(
                                    "http://www.sherpa.ac.uk/romeo/search.php?issn=" + journ.getIssn(), T_sherpa_more,
                                    "sherpaMoreInfo");
    ...
    /**
     * Each submission step must define its own information to be reviewed during
     * the final Review/Verify Step in the submission process.
     * <P>
     * The information to review should be tacked onto the passed in List object.
     * <P>
     * NOTE: To remain consistent across all Steps, you should first add a sub-List
     * object (with this step's name as the heading), by using a call to
     * reviewList.addList(). This sublist is the list you return from this method!
     *
     * @param reviewList The List to which all reviewable information should be
     *                   added
     * @return The new sub-List object created by this step, which contains all the
     *         reviewable information. If this step has nothing to review, then
     *         return null!
     * @throws org.xml.sax.SAXException                passed through.
    ...
    private void renderUploadedFileSummary(java.util.List<Bitstream> bitstreams, Division div, Item item)

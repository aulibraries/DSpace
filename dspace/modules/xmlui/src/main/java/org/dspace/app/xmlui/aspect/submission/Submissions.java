...
public class Submissions extends AbstractDSpaceTransformer
{
    ...
    // Custom Messages
    protected static final Message ETD_T_column5 =
        message("xmlui.Submission.Submissions.ETD_submit_column5");
    protected static final Message ETD_T_column6 =
        message("xmlui.Submission.Submissions.ETD_submit_column6");
        ...
        throws SQLException, WingException, IOException, AuthorizeException
    {
    	...
    	if (unfinishedItems.length <= 0 && supervisedItems.length <= 0)
    	{
            ...
            if (collections.length > 0)
            {
                ...
                p.addXref(contextPath+"/handle/"+collections[0].getHandle()+"/submit",T_s_info1b);
        ...
        header.addCellContent(ETD_T_column5);
        header.addCellContent(ETD_T_column6);
        ...
        if (unfinishedItems.length > 0)
        {
            for (WorkspaceItem workspaceItem : unfinishedItems)
            {
                ...
                Metadatum[] titles = workspaceItem.getItem().getMetadata(MetadataSchema.DC_SCHEMA, "title", null, Item.ANY);
                ...
                List<Metadatum> reasonMDVs = workspaceItem.getItem().getMetadata(MetadataSchema.DC_SCHEMA, "description", "provenance", Item.ANY, Item.ANY);
                ...
                if(!reasonMDVs.isEmpty() && reasonMDVs.size() > 0)
                {
                    if(reasonMDVs.contains("Rejected"))
                    {
                ...
                reasonMsg = getProvDescMDV(workspaceItem.getItem());
                if(reasonMsg != null && !(reasonMsg.isEmpty()))
                {
                    reasonCell.addContent(reasonMsg);
                }
            }
        }
        else
        ...
        throws SQLException,WingException
    {
        ...
        while(i.hasNext())
        {
            ...
            Metadatum[] titles = published.getMetadata(MetadataSchema.DC_SCHEMA, "title", null, Item.ANY);
            String collectionName = published.getOwningCollection().getName();
            Metadatum[] ingestDate = published.getMetadata(MetadataSchema.DC_SCHEMA, "date", "accessioned", Item.ANY);

            Row row = table.addRow();

            // Item accession date
            if (ingestDate != null && ingestDate.length > 0 && ingestDate[0].value != null)
            {
        ...
        throws SQLException, IOException, AuthorizeException
    {
        ...
        List<Metadatum> descProvMDVs = item.getMetadata(MetadataSchema.DC_SCHEMA, "description", "provenance", Item.ANY, Item.ANY);

        if(!descProvMDVs.isEmpty() && descProvMDVs.size() > 0)
        {
            String rejectProvMsg = null;
            for(Metadatum provMsgMDV : descProvMDVs)
            {
                if(provMsgMDV.value.startsWith("Rejected"))
                {
                    rejectProvMsg = provMsgMDV.value;
                }
            }

            if(rejectProvMsg != null)
            {
                int startIndex = rejectProvMsg.indexOf("reason:");
                int stopIndex = rejectProvMsg.indexOf(" # ");
                int start = startIndex + ("reason:".length()+1);

                if(stopIndex == -1)
                {
                    stopIndex = rejectProvMsg.length();
                }

                // Uncomment for debugging
                /*int yearIndex = rejectProvMsg.indexOf("2014");
                int msgLen = rejectProvMsg.length();

                log.debug(LogManager.getHeader(context, "Original Message", rejectProvMsg));
                log.debug(LogManager.getHeader(context, "", " IndexOf reason = "+String.valueOf(reasonIndex)));
                log.debug(LogManager.getHeader(context, "", " IndexOf on = "+String.valueOf(onIndex)));
                log.debug(LogManager.getHeader(context, "", " IndexOf 2014 = "+String.valueOf(yearIndex)));

                log.debug(LogManager.getHeader(context, "", " Message Length = "+String.valueOf(msgLen)));
                log.debug(LogManager.getHeader(context, "", " Value of start = "+String.valueOf(start)));
                log.debug(LogManager.getHeader(context, "", " Value of stop = "+String.valueOf(stop)));
                log.debug(LogManager.getHeader(context, "Char at stop", " = "+rejectProvMsg.charAt(stop)));*/

                reasonMsg = rejectProvMsg.substring(start, stopIndex);
            }
        }
        return reasonMsg;
    }

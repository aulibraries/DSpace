...
public class LDAPLogin extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
    ...
    /**
     * Generate the unique caching key. This key must be unique inside the space of
     * this component.
     */
     ...
     */
    public void addBody(Body body) throws AuthorizeException, IOException, SQLException, SAXException, WingException {
        ...
        if (StringUtils.isNotBlank(bitstreamIDStr)) {
            ...
            if (parent instanceof org.dspace.content.Item) {
                ...
                java.util.List<MetadataValue> embargoRightsList = itemService.getMetadata(item,
                        MetadataSchema.DC_SCHEMA,"rights", null, org.dspace.content.Item.ANY);
        ...
        // Display the login form when an item's bitstream is not restricted or is
        // restricted to authenticated Auburn users.
        if (StringUtils.isBlank(embargoRights)
                || authorizeService.isAccessRestrictedToNonAdminAuburnUsers(context, item)) {
            Division login = body.addInteractiveDivision("login", contextPath + "/ldap-login", Division.METHOD_POST,
                    "primary");
        ...
        if (StringUtils.isNotBlank(qStr)) {
            if (qStr.contains("=")) {
                ...
                if (qStrArray.length > 1) {
                    if ((StringUtils.isNotBlank(qStrArray[0]) && qStrArray[0].equals("bitstreamId"))
                            && StringUtils.isNotBlank(qStrArray[1])) {

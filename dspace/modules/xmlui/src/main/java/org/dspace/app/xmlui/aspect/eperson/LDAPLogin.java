...
public class LDAPLogin extends AbstractDSpaceTransformer implements CacheableProcessingComponent
{
        ...
        throws WingException
    {
        ...
        if(qStr != null)
        {
            ...
            if(qStrArray[1] != null)
            {
                bitstreamID = Integer.parseInt(qStrArray[1]);
            }
        }
        ...
        if(bitstreamID > 0)
        {
            try
            {
                org.dspace.content.Item item = getItemFromBitstream(context, bitstreamID);
                embargoRights = EmbargoManager.getEmbargoRightsMDV(context, item);
            }
            catch(SQLException | IOException | AuthorizeException excp)
            {
                log.error(LogManager.getHeader(context, "Getting Embargo Rights Information", excp.toString()));
            }
        }
        ...
        throws SQLException, SAXException, WingException
    {
        ...
        if(bitstreamID > 0)
        {
            ...
            catch(IOException | AuthorizeException excp)
            {
                log.error(LogManager.getHeader(context, "Getting Embargo Rights Information", excp.toString()));
            }
        }

        if(embargoRights == null || ETDEmbargoSetter.EMBARGO_NOT_AUBURN_STR.equals(embargoRights))
        {

...
public class EditItemStatusForm extends AbstractDSpaceTransformer 
{
    ...
    /* Custom Buttons */
    private static final Message T_submit_edit_embargo = message("xmlui.administrative.item.EditItemStatusForm.submit_edit_embargo");
    private static final Message T_label_edit_embargo = message("xmlui.administrative.item.EditItemStatusForm.label_edit_embargo");
        ...
        throws SQLException, WingException, IOException
    {
        ...
        {
            subList.addItem(collections[0].getName());
        }
        ...
        try
        {
            Bitstream bs = getItemBitstream(item);
            AuthorizeUtil.authorizeManageBitstreamPolicy(context, bs);
            itemInfo.addItem().addButton("submit_edit_embargo").setValue(T_submit_edit_embargo);
            itemInfo.addItem().addHidden("bitstreamID").setValue(bs.getID());
        }
        catch(Exception authex)
        {
            addNotAllowedButton(itemInfo.addItem(), "submit_edit_embargo", T_submit_edit_embargo);
        }
     ...
     * @throws SQLException 
     */
    private static Bitstream getItemBitstream(Item item)
        throws AuthorizeException, IOException, SQLException
    {
        ...
        {
            Bitstream[] bitstreams = bndl.getBitstreams();

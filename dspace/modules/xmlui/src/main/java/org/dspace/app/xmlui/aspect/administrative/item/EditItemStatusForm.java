...
public class EditItemStatusForm extends AbstractDSpaceTransformer {
    ...
    private static final Message T_label_modified = message(
            "xmlui.administrative.item.EditItemStatusForm.label_modified");
    ...
    private static final Message T_label_withdraw = message(
            "xmlui.administrative.item.EditItemStatusForm.label_withdraw");
    private static final Message T_label_reinstate = message(
            "xmlui.administrative.item.EditItemStatusForm.label_reinstate");
    ...
    private static final Message T_submit_authorizations = message(
            "xmlui.administrative.item.EditItemStatusForm.submit_authorizations");
    private static final Message T_submit_withdraw = message(
            "xmlui.administrative.item.EditItemStatusForm.submit_withdraw");
    private static final Message T_submit_reinstate = message(
            "xmlui.administrative.item.EditItemStatusForm.submit_reinstate");
    ...
    private static final Message T_submit_delete = message(
            "xmlui.administrative.item.EditItemStatusForm.submit_delete");
    ...
    private static final Message T_collectionadmins_only = message(
            "xmlui.administrative.item.EditItemStatusForm.collection_admins_only");

    private static final Message T_label_private = message(
            "xmlui.administrative.item.EditItemStatusForm.label_private");
    ...
    private static final Message T_submit_private = message(
            "xmlui.administrative.item.EditItemStatusForm.submit_private");
    private static final Message T_submit_public = message(
            "xmlui.administrative.item.EditItemStatusForm.submit_public");

    /* Custom Buttons */
    private static final Message AUETD_EDIT_EMBARGO_SUBMIT_BUTTON_NAME = message("xmlui.administrative.item.EditItemStatusForm.AUETD_edit_embargo_submit_button_name");
    private static final Message AUETD_EDIT_EMBARGO_SECTION_LABEL = message("xmlui.administrative.item.EditItemStatusForm.AUETD_edit_embargo_section_label");
    ...
    public void addBody(Body body) throws SQLException, WingException {
        ...
        Division main = body.addInteractiveDivision("edit-item-status", contextPath + "/admin/item",
                Division.METHOD_POST, "primary administrative edit-item-status");
        ...
        if (item.getHandle() == null) {
            ...
            itemInfo.addItem().addXref(
                    DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("dspace.url") + "/handle/"
                            + item.getHandle(),
                    DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("dspace.url") + "/handle/"
                            + item.getHandle());
        }
         ...
         * screen.
         */
        itemInfo.addLabel(AUETD_EDIT_EMBARGO_SECTION_LABEL);
        try {
            int numBitstreams = getNumItemBitstreams(item);

            if (numBitstreams == 1) {
                Bitstream bitstream = getItemBitstream(item);
                AuthorizeUtil.authorizeManageBitstreamPolicy(context, bitstream);
                itemInfo.addItem().addButton("submit_edit_embargo").setValue(AUETD_EDIT_EMBARGO_SUBMIT_BUTTON_NAME);
                main.addHidden("bitstreamID").setValue(bitstream.getID().toString());
                main.addHidden("numBitstreams").setValue(String.valueOf(numBitstreams));
            }
        } catch (AuthorizeException authex) {
            addNotAllowedButton(itemInfo.addItem(), "submit_edit_embargo", AUETD_EDIT_EMBARGO_SUBMIT_BUTTON_NAME);
        }
     ...
     * @throws SQLException
     */
    private void addNotAllowedButton(org.dspace.app.xmlui.wing.element.Item item, String buttonName,
            Message buttonLabel) throws WingException, SQLException {
    ...
    private void addCollectionAdminOnlyButton(org.dspace.app.xmlui.wing.element.Item item, Collection collection,
            String buttonName, Message buttonLabel) throws WingException, SQLException {
        Button button = item.addButton(buttonName);
        button.setValue(buttonLabel);
    ...
    private int getNumItemBitstreams(Item item) throws AuthorizeException, SQLException
    {
        int numFiles = 0;

        for (Bundle bundle : itemService.getBundles(item, Constants.CONTENT_BUNDLE_NAME)) {
            numFiles = bundle.getBitstreams().size();
        }
        return numFiles;
    }

    private Bitstream getItemBitstream(Item item) throws AuthorizeException, SQLException
    {
        Bitstream bitstream = null;

        for (Bundle bundle : itemService.getBundles(item, Constants.CONTENT_BUNDLE_NAME)) {
            if (bundle.getBitstreams().size() > 0) {
                bitstream = bundle.getBitstreams().get(0);
            }
        }

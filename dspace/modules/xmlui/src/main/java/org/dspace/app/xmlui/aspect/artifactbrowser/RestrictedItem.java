...
public class RestrictedItem extends AbstractDSpaceTransformer //implements CacheableProcessingComponent
{
                ...
                SQLException, IOException, AuthorizeException 
    {
        ...
        if(dso != null)
        {
            ...
            {
                pageMeta.addMetadata("nav", "view-account-nav").addContent("false");
            }
        }
        else
        {
            if(request.getQueryString() != null)
            {
                ...
                if(qStrArray.length > 0)
                {
                    if(qStrArray[0].startsWith("bitstream"))
                    {
                        pageMeta.addMetadata("nav", "view-account-nav").addContent("false");
                    }
                }
            ...
            IOException, AuthorizeException, ResourceNotFoundException 
    {
        ...
        else if (dso instanceof Item) 
        {
            ...
            if (request.getParameter("bitstreamId") != null) {
                ...
                Metadatum[] titles = dso.getMetadata(MetadataSchema.DC_SCHEMA, "title", null, Item.ANY);

...
public class RestrictedItem extends AbstractDSpaceTransformer //implements CacheableProcessingComponent
{
            ...
            AuthorizeException
    {
        ...
        {
            org.dspace.content.Item item = getItemFromBitstream(context, Integer.parseInt(request.getParameter("bitstreamId")));
            embargoRights = EmbargoManager.getEmbargoRightsMDV(context, item);
        }

        if(embargoRights != null && ETDEmbargoSetter.EMBARGO_GLOBAL_STR.equals(embargoRights))
        {
    ...
    public void addBody(Body body)
        throws SAXException, WingException, UIException, SQLException,
                IOException, AuthorizeException, ResourceNotFoundException
    {
        Request request = ObjectModelHelper.getRequest(objectModel);
        DSpaceObject dso = HandleUtil.obtainHandle(objectModel);
        ...
        else if (dso instanceof Item)
        {
            ...
            if (request.getParameter("bitstreamId") != null)
            {
                ...
                try
                {
                    bit = Bitstream.find(context, new Integer(request.getParameter("bitstreamId")));
                    if (bit != null)
                    {
                ...
                String returnURL = request.getContextPath()+"/handle/"+dso.getHandle();
                unauthorized.addPara().addXref(returnURL, "<< Return");
            }
            else
            {
                String identifier = "unknown";
        ...
        String embargoRights = EmbargoManager.getEmbargoRightsMDV(context, item);

        // add a login link if !loggedIn & not withdrawn
        if (!isWithdrawn && context.getCurrentUser() == null && ETDEmbargoSetter.EMBARGO_NOT_AUBURN_STR.equals(embargoRights))
        {
     ...
     * @throws org.dspace.authorize.AuthorizeException
     */
    private static org.dspace.content.Item getItemFromBitstream(Context context, int bsID)

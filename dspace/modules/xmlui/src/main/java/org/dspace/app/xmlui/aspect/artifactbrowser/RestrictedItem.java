...
public class RestrictedItem extends AbstractDSpaceTransformer // implements CacheableProcessingComponent
{
    ...
    private static final Message T_head_item_withdrawn = message(
            "xmlui.ArtifactBrowser.RestrictedItem.head_item_withdrawn");
    ...
    // Item states
    private static final Message T_para_item_restricted_auth = message(
            "xmlui.ArtifactBrowser.RestrictedItem.para_item_restricted_auth");
    private static final Message T_para_item_restricted = message(
            "xmlui.ArtifactBrowser.RestrictedItem.para_item_restricted");
    private static final Message T_para_item_withdrawn = message(
            "xmlui.ArtifactBrowser.RestrictedItem.para_item_withdrawn");
    ...
    public void addBody(Body body) throws SAXException, WingException, UIException, SQLException, IOException,
            AuthorizeException, ResourceNotFoundException {
        Request request = ObjectModelHelper.getRequest(objectModel);
        DSpaceObject dso = HandleUtil.obtainHandle(objectModel);
        ...
        if (dso == null) {
            ...
            // The dso may be an item but it could still be an item's bitstream. So let's
            // check for the parameter.
            if (StringUtils.isNotBlank(request.getParameter("bitstreamId"))) {
                String identifier = "unknown";
                try {
                    Bitstream bit = bitstreamService.find(context,
                            UUID.fromString(request.getParameter("bitstreamId")));
                    if (bit != null) {
            ...
            // This case should not occur, but if it does just fall back to the resource
            // message.
            unauthorized = body.addDivision("unauthorized-resource", "primary");
            unauthorized.setHead(T_head_resource);
            unauthorized.addPara(T_para_resource);
        ...
        // Interrupt the request if the current user is not loggedIn, and the item is
        // not withdrawn

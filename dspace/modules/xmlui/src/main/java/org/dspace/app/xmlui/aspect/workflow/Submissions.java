...
public class Submissions extends AbstractDSpaceTransformer
{
    ...
    // Custom messages
    protected static final Message others_workflow_head =
        message("xmlui.Submission.Submissions.others_workflow_head");
    protected static final Message T_others_w_column1 =
        message("xmlui.Submission.Submissions.others_workflow_column1");
    protected static final Message T_others_w_column2 =
        message("xmlui.Submission.Submissions.others_workflow_column2");
    protected static final Message T_others_w_column3 =
        message("xmlui.Submission.Submissions.others_workflow_column3");
    protected static final Message T_others_w_column4 =
        message("xmlui.Submission.Submissions.others_workflow_column4");
        ...
        throws SQLException, WingException
    {
        ...
        List<WorkflowItem> othersItems = WorkflowManager.getAllOtherTasks(context);
        ...
        if (pooledItems.size() > 0)
        {
            for (WorkflowItem pooled : pooledItems)
            {
                ...
                if(wfiOwner != null)
                {
                    if(wfiOwner == context.getCurrentUser())
                    {
                        continue;
                    }
                    else
                    {
                        ownerName = wfiOwner.getFullName();
                    }
                }
        ...
        table = workflow.addTable("workflow-tasks",othersItems.size()+2,5);
        table.setHead(others_workflow_head);

        header = table.addRow(Row.ROLE_HEADER);
        header.addCellContent(T_others_w_column1);
        header.addCellContent(T_others_w_column2);
        header.addCellContent(T_others_w_column3);
        header.addCellContent(T_others_w_column4);

        if(othersItems.size() > 0)
        {
            for(WorkflowItem othersItem : othersItems)
            {
                int workflowItemID = othersItem.getID();
                String collectionUrl = contextPath + "/handle/" + othersItem.getCollection().getHandle();
                Metadatum[] titles = othersItem.getItem().getMetadata(MetadataSchema.DC_SCHEMA, "title", null, Item.ANY);
                String collectionName = othersItem.getCollection().getName();
                String ownerName = othersItem.getOwner().getFullName();
                String ownerEmail = othersItem.getOwner().getEmail();
                Message state = getWorkflowStateMessage(othersItem);

                if(othersItem.getOwner() != context.getCurrentUser())
                {
                    Row row = table.addRow();
                    ...
                    if (titles != null && titles.length > 0)
                    {
                        ...
                        row.addCell().addContent(displayTitle);
                    }
                    else
                    {
                        row.addCell().addContent(T_untitled);
                    }
                    ...
                    Cell cell = row.addCell();
                    cell.addContent(T_email);
                    cell.addXref("mailto:"+ownerEmail, ownerName);
                }
            }
        }
        else

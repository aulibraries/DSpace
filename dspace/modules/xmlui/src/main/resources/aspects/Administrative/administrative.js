...
function doEditItemStatus(itemID) {
    ...
    do {
        ...
        } else if (cocoon.request.get("submit_edit_embargo")) {
            var numBitstreams = parseInt(cocoon.request.get("numBitstreams"));
            if (numBitstreams > 1) {
                doEditItemBitstreams(itemID);
            } else {
                doEditBitstream(itemID, cocoon.request.get("bitstreamID"));
            }
        }
    } while (true)
}
...
function doEditBitstream(itemID, bitstreamID) {
    var bitstreamUUID = UUID.fromString(bitstreamID);
    var itemUUID = UUID.fromString(itemID);
    assertAuthorized(Constants.BITSTREAM, bitstreamUUID, Constants.WRITE);
    var result;
    do {
        ...
        if (cocoon.request.get("submit_cancel")) {
            ...
            result = FlowItemUtils.processEditBitstream(getDSContext(), itemUUID, bitstreamUUID, bitstreamName, primary, description, formatID, userFormat, cocoon.request);
        }
    } while (result == null || !result.getContinue())

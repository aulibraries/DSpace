...
<xsl:stylesheet
    ...
    <xsl:template match="dri:list[@id='aspect.browseArtifacts.CommunityBrowse.list.community-browse' or @id='aspect.browseArtifacts.CollectionBrowse.list.collection-browse']/dri:item[dri:xref]">
        <xref>
            <xsl:call-template name="copy-attributes"/>
            <xsl:attribute name="rend">
    ...
    <xsl:template match="dri:field[@id='aspect.administrative.WithdrawnItems.field.rpp' or @id='aspect.administrative.PrivateItems.field.rpp']/dri:option">
       <xsl:if test="@returnValue=5 or @returnValue=10 or @returnValue=20 or @returnValue=40 or @returnValue=60 or @returnValue=80 or @returnValue=100">
            <option>
                <xsl:call-template name="copy-attributes"/>
                <xsl:apply-templates />
            </option>

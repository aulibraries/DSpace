...
-->
                            ...
                            <xsl:when test="local-name() = 'text' and following-sibling::*[local-name() = 'field']">
                                <label>
                                    <xsl:attribute name="class">
                                        <xsl:text>ds-form-label control-label</xsl:text>
                                    </xsl:attribute>
                                    <xsl:attribute name="for">
                                        <xsl:value-of select="translate(following-sibling::dri:field/@id, '.', '_')" />
                                    </xsl:attribute>
                                    <xsl:apply-templates select="." />
                                </label>
                                <xsl:apply-templates select="dri:field" mode="normalField" />
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:apply-templates select="." />
                            </xsl:otherwise>
    ...
    <xsl:template match="dri:div[@n = 'epeople-actions' or @n='group-actions' or @n='authorization-actions']">
        <xsl:apply-templates select="dri:head" />
        <div>
            <xsl:call-template name="standardAttributes">
                <xsl:with-param name="class">
                    <xsl:text>col-lg-12 searchActionsBlock</xsl:text>
                </xsl:with-param>
            </xsl:call-template>
            <xsl:apply-templates select="dri:list[@n='actions']/dri:item" mode="ActionsListItem" />
        </div>
    </xsl:template>
    ...
    <xsl:template name="renderHead">
        ...
        <xsl:variable name="tagName" select="$head_count" />
        <xsl:element name="h{$tagName}">
            <xsl:call-template name="standardAttributes">
                <xsl:with-param name="class">
